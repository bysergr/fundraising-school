...
export default function FundModal() {
  ...
  const fund_rounds = modal_vc.rounds.map((round) => round?.stage);
  const fund_check_size = modal_vc.check_size.map((check_size) => check_size?.size);
  const fund_sectors = modal_vc.sectors.map((sector) => sector?.name);
  const fund_countries = modal_vc.countries.map((country) => country?.name);

  return (
    <dialog
      className="left-1/2 top-0 m-0 h-screen w-1/2 max-w-[763px] overflow-y-auto bg-neutral-50 px-14 py-8"
      ref={dialogRef}
      onCancel={closeModal}
    >
